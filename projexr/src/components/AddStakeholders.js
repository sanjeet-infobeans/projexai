...
const sampleStakeholders = [
  { name: 'Ethan Carter', role: 'Project Manager' },
  { name: 'Olivia Bennett', role: 'IT Director' },
  { name: 'Noah Thompson', role: 'Security Analyst' },
  { name: 'Ava Harper', role: 'System Administrator' },
  { name: 'Liam Foster', role: 'Network Engineer' },
];

const AddStakeholders = () => {
  const [search, setSearch] = useState('');
  // For demo, filter by name or role (case-insensitive)
  const filtered = sampleStakeholders.filter(s =>
        ...
        <div className="px-4 py-3 @container">
          <div className="flex overflow-hidden rounded-xl border border-[#dcdee5] bg-white">
            <table className="flex-1">
              ...
              <tbody>
                {filtered.map((s, idx) => (
                  <tr className="border-t border-t-[#dcdee5]" key={s.name}>
          ...
          {/* Responsive styles for columns (optional) */}
        </div>
      </div>
    </div>
  );
